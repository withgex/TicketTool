            ...
            if (button.id == 'createTicket') {
                ...
                if (checkTickets) {
                    button.channel.send({
                        embed: {
                            color: 0xFF0000,
                            title: `**❌ | Error**`,
                            description: `Ya tienes un ticket abierto anteriormente`
                        }
                ...
                button.guild.channels.create(`ticket-${button.clicker.user.username}`, {
                    ...
                    channel.send(`<@${button.clicker.user.id}>`, {
                        embed: {
                            description: `¡Espere una respuesta de algún ** ADMIN **!
                    Presiona **"🔒"** para cerrar el ticket`,
                            color: 0x2F3136
                        },
                        component: row
                    });
                ...
                button.channel.send({
                    embed: {
                        description: `El Ticket ha sido cerrado por <@!${button.clicker.user.id}>`,
                        color: 0xFFE700
                    }
                }).then(async function(m) {
                    setTimeout(() => {
                ...
                button.channel.send({
                    embed: {
                        description: 'Presiona **"⛔"** para eliminar el ticket\nPresiona **"🔓"** para reabrir de nuevo el ticket',
                        color: 0xFF0000
                    },
                    component: row
                }).then(async function(m) {
                ...
                button.channel.send({
                    embed: {
                        description: 'El ticket se eliminará en unos segundos',
                        color: 0xFF0000
                    }
                });
                setTimeout(() => {
                ...
                button.channel.send({
                    embed: {
                        description: `El ticket ha sido reabierto por <@!${button.clicker.user.id}>`,
                        color: 0xFFE700
                    }
                }).then(async function(m) {
                    setTimeout(() => {
                ...
                button.channel.send({
                            embed: {
                                title: '🎫',
                                description: `Se ha cambiado el nombre de este ticket a \`${require('quick.db').fetch(`RenameTicket_${button.channel.id}`)}\``,
                    color: 0x00D700
                }
            })
            require('quick.db').delete(`DeleteRenameMessage_${button.channel.id}`);
